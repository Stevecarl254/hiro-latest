...
const categories = [
    {
        name: "Tents",
        icon: <FaChair />,
        btn: "bg-[#FF6600] text-white border-[#FF6600]",
        badge: "bg-[#FFE5D1] text-[#FF6600]",
    },
    {
        name: "Chairs & Seating",
        icon: <FaChair />,
        btn: "bg-[#007BFF] text-white border-[#007BFF]",
        badge: "bg-[#D1E9FF] text-[#007BFF]",
    },
    {
        name: "Tables & Tableware",
        icon: <FaUtensils />,
        btn: "bg-[#22C55E] text-white border-[#22C55E]",
        badge: "bg-[#DCFCE7] text-[#22C55E]",
    },
    {
        name: "Decor & Linens",
        icon: <FaGift />,
        btn: "bg-[#9333EA] text-white border-[#9333EA]",
        badge: "bg-[#F3E8FF] text-[#9333EA]",
    },
    {
        name: "Stage & Lighting",
        icon: <FaGlassCheers />,
        btn: "bg-[#EC4899] text-white border-[#EC4899]",
        badge: "bg-[#FCE7F3] text-[#EC4899]",
    },
    {
        name: "Accessories",
        icon: <FaGift />,
        btn: "bg-[#F59E0B] text-white border-[#F59E0B]",
...
const HiringCateringClient = () => {
    ...
    const categoryHeaderRef = useRef<HTMLElement | null>(null);
    ...
    const filtered = equipment
        .filter((eq) => {
            ...
            if (searchTerm.trim() !== "") {
                return matchesSearch; // Global search across all categories
            }
            return eq.category === selectedCategory; // Filtered by category if no search term
        })
        .sort((a, b) => a.name.localeCompare(b.name));

    const modalFiltered = equipment
        .filter((eq) => {
            const matchesSearch = eq.name.toLowerCase().includes(modalSearch.toLowerCase());

            if (modalSearch.trim() !== "") {
                return matchesSearch; // Global search in modal
            }
    ...
    const handleEquipmentClick = (category: string) => {
        if (searchTerm.trim() !== "" || selectedCategory !== category) {
            setSelectedCategory(category);
            setSearchTerm("");
            setCurrentPage(1);
            categoryHeaderRef.current?.scrollIntoView({ behavior: "smooth" });
        }
    };
    ...
    return (
        <main className="min-h-screen py-16 px-6 md:px-20 bg-gray-50 relative overflow-hidden">
            ...
            <section
                ...
                className="text-center mb-8 relative z-10 scroll-mt-24"
            >
                <h2 className="text-3xl font-bold text-[#FF6600]">
                    {searchTerm.trim() !== "" ? `Search Results for "${searchTerm}"` : `${selectedCategory} Equipment`}
                </h2>
            </section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {paginated.map((eq) => (
                            <motion.div
                                ...
                                onClick={() => handleEquipmentClick(eq.category)}
                                className="bg-white rounded-2xl shadow-md p-6 border border-gray-100 relative h-fit cursor-pointer"
                            >
                                {eq.imageUrl ? (
                                    ...
                                    <div className="relative h-40 mb-3 overflow-hidden rounded-xl bg-gray-100 flex items-center justify-center">
                                        <img
                    ...
                    <div className="bg-white rounded-2xl shadow-lg w-full max-w-2xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="p-4 md:p-6 border-t sticky bottom-0 bg-white z-10">
                            <button
                                ...
                                className="w-full bg-[#00b8e6] hover:bg-[#0095c7] text-white py-3 rounded-lg font-semibold disabled:opacity-50"
                            >
...
export default HiringCateringClient;
